...
class MarioRNN(object):
    def __init__(self, data, rnn_sizes, max_grad_norm, dropout_keep, variational_recurrent, train, loss_function):
        ...
        with tf.variable_scope("RNN"):
            self.layers = [tf.nn.rnn_cell.BasicLSTMCell(size, forget_bias=1.0, state_is_tuple=True) for size in
                           rnn_sizes]

        # Final layer of logistic activations for button presses
        self.logitsW = tf.Variable(
            tf.truncated_normal([rnn_sizes[-1], data.output_size], stddev=1.0 / math.sqrt(float(data.input_size))),
            name="LogitsW")
